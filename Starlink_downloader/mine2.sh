...
main() {
    ...
    if [[ -n "$MANIFEST_PATH_INPUT" ]]; then
        ...
        if curl -A "$USER_AGENT" --fail --silent --show-error -o "${manifest_path}.tmp" "$MANIFEST_URL"; then
    ...
    local manifest_files=($(ls $LOG_DIR/MANIFEST_*.txt 2>/dev/null))
    ...
    if [[ $TOTAL_FILES -gt 0 ]]; then
        ...
        local download_start_time=$(date +%s)   # 下载起始时刻
        local prev_completed_time=$(date +%s)   # 上次完成下载的时间
        local prev_completed_count=0    # 上次完成下载的数量
        local i=0
        while [[ $i -lt $TOTAL_FILES ]]; do
            local batch_completed_count=0
            # local -A pids_files # 进程文件映射版
            while [[ ${#BACKGROUND_PIDS[@]} -lt $THROTTLE_LIMIT ]] && [[ $i -lt $TOTAL_FILES ]]; do
                download_file "${new_files[i]}" "$download_dir" &
                local pid=$!
                BACKGROUND_PIDS+=("$pid")
                # pids_files["$pid"]="${new_files[i]}"  # 进程文件映射版
                ((i++))
            done
            ...
            # for pid in "${BACKGROUND_PIDS[@]}"; do
            #     if ! kill -0 "$pid" 2>/dev/null; then
            #         if [[ -s "$download_dir/${pids_files[$pid]}" ]]; then
            #             ((batch_completed_count++))
            #         fi
            #     else
            #         active_pids+=("$pid") # 将仍然活跃的 PID 放入新数组
            #     fi
            # done
            # BACKGROUND_PIDS=("${active_pids[@]}") # 更新活跃的 PID 列表，进程文件映射版
            ...
            local delta=$((COMPLETED_COUNT - prev_completed_count))
            if (( delta >= THROTTLE_LIMIT )); then
                show_progress "$TOTAL_FILES" "$delta" "$COMPLETED_COUNT" "$prev_completed_time" "$download_start_time"
                prev_completed_time=$(date +%s)
        ...
        # 修复索引
        BACKGROUND_PIDS=("${BACKGROUND_PIDS[@]}")
        show_progress "$TOTAL_FILES" "$((COMPLETED_COUNT - prev_completed_count))" "$COMPLETED_COUNT" "$prev_completed_time" "$download_start_time"
        prev_completed_time=$(date +%s)
        prev_completed_count=$COMPLETED_COUNT
